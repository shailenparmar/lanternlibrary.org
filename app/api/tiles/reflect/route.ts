...
import { liveContributorTiles } from "@/lib/llm";

export const runtime = "nodejs";

const MAX_DRAFT_LEN = 12000;
const MIN_DRAFT_LEN = 20;
...
export async function POST(req: Request) {
  ...
  if (draft.trim().length < MIN_DRAFT_LEN) {
    return NextResponse.json({ phrases: [], invitations: [] });
  }
  try {
    const tiles = await liveContributorTiles(draft);
    ...
    console.error("reflect tiles error:", err);
